...
# #Adding our first GRUlayer

gru_rnn.add(GRU(units = 45, return_sequences = True, input_shape = (x_training_data.shape[1], 1)))

#Perform some dropout regularization

gru_rnn.add(Dropout(0.2))

#Adding three more GRU layers with dropout regularization

for i in [True, True, False]:

    gru_rnn.add(GRU(units = 45, return_sequences = i))

    gru_rnn.add(Dropout(0.2))
    
#Adding our output layer

gru_rnn.add(Dense(units = 1, activation='sigmoid')) # CHANGE activation='linear' for BONUS TASK and change target label to KIBA column

gru_rnn.summary()

#Compiling the recurrent neural network
gru_rnn.compile(optimizer = 'adam', loss = 'binary_crossentropy', metrics=['acc'])

# Create callbacks: Early Stopping Pt and check point to later reload the trained classifier
callbacks=[]
es=EarlyStopping(monitor='val_loss', patience=50)
callbacks.append(es)
mcp=ModelCheckpoint('../gru_rnn_models/gru_rnn_model.h5', save_best_only=True, save_weights_only=False)
callbacks.append(mcp)



#Training the recurrent neural network
classifierTypeName="GRU RNN"
numFeat="5"
...
train_gru_rnn=train_evaluate_model(dfX_train, dfY_train, gru_rnn, classifierTypeName, numFeat, num_epochs, num_bsize, valid_dataset)

model_trained=train_gru_rnn["model_trained"]

# save model and architecture to single file
model_trained.save("gru_rnn_model_last.h5")
