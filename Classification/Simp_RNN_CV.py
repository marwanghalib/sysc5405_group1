...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 25 06:09:06 2021

@author: PalwashaW_Shaikh
"""

#SIMPLE RNN WITH 5-CV for Binary Classification____________________________________________________________________
import pandas as pd
...
def print_confusion_matrix(Y_test,Y_pred):
    ...
    plt.savefig('simple_rnn_confusion_matrix.png')
    plt.show()
    return cm
...
def plot_roc_CV(fpr_cv,tpr_cv,aucs,ns_fpr,ns_tpr,ns_auc,mean_tpr,mean_fpr,mean_auc,g_title):
    ...
    plt.savefig('simple_rnn_roc_curve.png')
    plt.show()
...
def plot_pr_re_curve_CV(precision_list,recall_list,pr_auc_list,no_skill,ns_pr_auc,mean_precision,mean_recall,mean_pr_auc,g_title):
    ...
    plt.legend(prop={'size': 10})
    plt.savefig('simple_rnn_pr_re_curve.png')
    plt.show()
...
def train_evaluate_model(data_x, data_y, classifier, classifierTypeName, numFeat, num_epochs, num_bsize,valid_dataset):
    ...
    for k, (train_ix, test_ix) in enumerate(kfold):
        ...
        print()
        cm=print_confusion_matrix(test_y,predicted_labels) #per fold
        #print(cm)
        print()
    ...
    print()
    print('5-CV Pr at Re 0.50: %.5f +/- %.20f' %(pr_re50_mean, pr_re50_std))
    print()
    ...
    #cm=print_confusion_matrix(predicted_targets,actual_targets) #concatenated
    ...
    plt.legend()
    plt.savefig('simple_rnn_training_history_loss.png')
    plt.show()
    ...
    plt.legend()
    plt.savefig('simple_rnn_training_history_acc.png')
    plt.show()
...
def test_dataset_evaluate(classifier, x_test_data, y_test_data, classifierTypeName, numFeats):
    ...
    plt.ylabel('True Positive Rate (Sensitivity)')
    plt.legend()
    plt.savefig('simple_rnn_roc_held_25_percent.png')
    plt.show()
    ...
    plt.legend()
    plt.savefig('simple_rnn_pr_re_held_25_percent.png')
    plt.show()
...
print(df)
print(df.head())
#No. of true and false labels
num_labels = df.groupby('Label').size() 
print(num_labels)
...
plt.savefig('simple_rnn_anova_top_25.png')
plt.show()
plt.clf()
...
simp_rnn = Sequential()
#Layers input shape (x_training_data.shape[1], 1)
simp_rnn.add(SimpleRNN(units=256, return_sequences = True, input_shape = (x_training_data.shape[1], 1)))
simp_rnn.add(Dropout(0.2))
simp_rnn.add(SimpleRNN(units=128, return_sequences = True))
simp_rnn.add(Dropout(0.2))
simp_rnn.add(SimpleRNN(units=64, return_sequences = False))
simp_rnn.add(Dropout(0.2))
#Output Layer - sigmoid for binary classification
simp_rnn.add(Dense(units = 1, activation='sigmoid')) # CHANGE activation='linear' for BONUS TASK and change target label to KIBA column

simp_rnn.summary()

#Compiling the recurrent neural network
simp_rnn.compile(optimizer = 'adam', loss = 'binary_crossentropy', metrics=['acc'])
...
classifierTypeName="Simple RNN"
numFeat="5"
num_epochs=100 #per fold - total folds are 5 So, 500
num_bsize=32
...
train_simp_rnn=train_evaluate_model(dfX_train, dfY_train, simp_rnn, classifierTypeName, numFeat, num_epochs, num_bsize, valid_dataset)

model_trained=train_simp_rnn["model_trained"]

# save model and architecture to single file
model_trained.save("simp_rnn_model_last.h5")
